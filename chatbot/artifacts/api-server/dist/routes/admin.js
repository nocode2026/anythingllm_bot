...
exports.adminRouter.get('/stats', async (_req, res) => {
    const [questions, today, unanswered, sources, chunks] = await Promise.all([
        index_1.pool.query(`SELECT COUNT(*) FROM chat_messages WHERE role='user'`),
        index_1.pool.query(`SELECT COUNT(*) FROM chat_messages WHERE role='user' AND created_at > NOW() - INTERVAL '1 day'`),
        index_1.pool.query(`SELECT COUNT(*) FROM chat_messages WHERE response_type='fallback'`),
        index_1.pool.query(`SELECT COUNT(*) FROM knowledge_sources`),
        index_1.pool.query(`SELECT COUNT(*) FROM knowledge_chunks`),
    ]);
    ...
    const lastSync = await index_1.pool.query(`SELECT MAX(last_indexed_at) AS last_indexed_at FROM knowledge_sources`);
    res.json({
...
exports.adminRouter.get('/unanswered', async (req, res) => {
    const limit = Math.min(parseInt(String(req.query.limit ?? '50'), 10), 200);
    const offset = parseInt(String(req.query.offset ?? '0'), 10);
    const { rows } = await index_1.pool.query(`SELECT m.id, m.content AS question, m.created_at, s.faculty_context
...
exports.adminRouter.get('/low-confidence', async (req, res) => {
    const threshold = parseFloat(String(req.query.threshold ?? '0.60'));
    const limit = Math.min(parseInt(String(req.query.limit ?? '50'), 10), 200);
    const { rows } = await index_1.pool.query(`SELECT id, content, final_answer_confidence, created_at
     FROM chat_messages
...
exports.adminRouter.get('/sources', async (_req, res) => {
    const { rows } = await index_1.pool.query(`SELECT id, source_type, source_url, scope, faculty_id,
...
exports.adminRouter.post('/reindex', async (_req, res) => {
    const jobId = (0, uuid_1.v4)();
    activeJobs.set(jobId, { status: 'running', started_at: new Date().toISOString() });
    await index_1.pool.query(`INSERT INTO ingestion_progress (job_id, status, started_at) VALUES ($1, 'running', NOW())
    ...
    setImmediate(async () => {
        try {
            ...
            await index_1.pool.query(`UPDATE ingestion_progress SET status = 'completed', completed_at = NOW() WHERE job_id = $1`, [jobId]);
        }
        catch (err) {
            activeJobs.set(jobId, { status: 'failed', started_at: activeJobs.get(jobId).started_at });
            await index_1.pool.query(`UPDATE ingestion_progress SET status = 'failed', error_message=$2 WHERE job_id = $1`, [jobId, String(err)]);
        }
    });
    res.json({ job_id: jobId, status: 'running' });
});
// GET /api/admin/reindex/:jobId
exports.adminRouter.get('/reindex/:jobId', async (req, res) => {
    const { jobId } = req.params;
    const { rows } = await index_1.pool.query(`SELECT * FROM ingestion_progress WHERE job_id = $1`, [jobId]);
    if (rows.length === 0) {
