...
const queryRouter_1 = require("../lib/queryRouter");
const retrieval_1 = require("../lib/retrieval");
const answerPipeline_1 = require("../lib/answerPipeline");
const sessionManager_1 = require("../lib/sessionManager");
const moderationGuard_1 = require("../lib/moderationGuard");
const analytics_1 = require("../lib/analytics");
const suggestedQuestions_1 = require("../lib/suggestedQuestions");
exports.chatRouter = (0, express_1.Router)();
function stripSuggestedSection(answerText) {
...
const MessageBodySchema = zod_1.z.object({
    ...
    faculty_override: zod_1.z.string().optional(),
});
...
exports.chatRouter.post('/message', async (req, res) => {
    ...
    // ── 1. Moderation: input ──
    const inputMod = (0, moderationGuard_1.moderateInput)(message);
    if (inputMod.blocked) {
        return res.status(200).json({
            session_id: session_id ?? null,
            answer: (0, moderationGuard_1.getModerationResponse)(inputMod.reason),
    ...
    // ── 2. Session ──
    const session = await (0, sessionManager_1.getOrCreateSession)(session_id);
    const effectiveFaculty = faculty_override ?? session.faculty_context ?? null;
    // ── 3. Query classification ──
    const classification = (0, queryRouter_1.classifyQuery)(message, effectiveFaculty);
    ...
    if (classification.is_follow_up && session.last_message_type) {
        if ((0, sessionManager_1.canInheritContext)(session)) {
            // Full inheritance
        ...
        else if ((0, sessionManager_1.canInheritContextCautious)(session)) {
            resolvedFaculty = session.last_resolved_faculty_id;
            resolvedScope = session.last_resolved_scope ?? classification.scope;
            if (resolvedTopicTags.length === 0 && session.last_resolved_topic_tags.length > 0) {
                resolvedTopicTags = [...session.last_resolved_topic_tags];
    ...
    const { FACULTY_PROGRAMS } = await Promise.resolve().then(() => __importStar(require('../lib/queryRouter')));
    ...
    // ── 5. Save user message ──
    const userMsgId = await (0, sessionManager_1.saveMessage)(session.id, {
        role: 'user',
        content: message,
    ...
    let embedding;
    try {
        embedding = await (0, answerPipeline_1.getEmbedding)(message);
    }
    catch (err) {
    ...
    // ── 7. Retrieval ──
    const retrieval = await (0, retrieval_1.retrieveChunks)(message, embedding, resolvedScope, resolvedFaculty, resolvedTopicTags);
    // ── 8. Answer Pipeline ──
    const answer = await (0, answerPipeline_1.generateAnswer)(message, retrieval.chunks, retrieval.retrieval_confidence, resolvedScope, resolvedFaculty, resolvedScope === 'general');
    const isGeneralLegitymacja = resolvedScope === 'general' && resolvedTopicTags.includes('legitymacja');
    ...
    const outputMod = (0, moderationGuard_1.moderateOutput)(answer.answer_text);
    ...
    const assistantMsgId = await (0, sessionManager_1.saveMessage)(session.id, {
        role: 'assistant',
    ...
    await (0, sessionManager_1.updateSession)(session.id, {
    ...
    const suggestedQuestions = (0, suggestedQuestions_1.buildSuggestedQuestions)({
...
exports.chatRouter.get('/session/:sessionId', async (req, res) => {
    try {
        const messages = await (0, sessionManager_1.getSessionMessages)(req.params.sessionId);
...
exports.chatRouter.delete('/session/:sessionId', async (req, res) => {
    await (0, sessionManager_1.resetSession)(req.params.sessionId);
    res.json({ ok: true });
});
