...
function inferTopic(currentTopicTags, previousTopicTags, answerText) {
    for (const tag of currentTopicTags) {
        if (TOPIC_SUGGESTIONS[tag])
            return tag;
    }
    ...
    if (lower.includes('stypend'))
        return 'stypendium';
    if (lower.includes('ubezpiec'))
        return 'ubezpieczenie';
    if (lower.includes('erasmus'))
        return 'erasmus';
    if (lower.includes('praktyk'))
        return 'praktyki';
    if (lower.includes('legitymac'))
        return 'legitymacja';
    if (lower.includes('dziekanat'))
        return 'dziekanat';
    return null;
}
function avoidRepeatingUserIntent(candidates, userMessage) {
    const m = userMessage.toLowerCase();
    return candidates.filter((q) => !m.includes(q.toLowerCase().slice(0, 16)));
}
function withScopeHint(questions, scope, facultyId) {
