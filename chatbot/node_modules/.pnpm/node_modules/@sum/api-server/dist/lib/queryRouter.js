...
exports.TOPIC_ALIASES = exports.GENERAL_TOPICS = exports.FACULTY_PROGRAMS = exports.FACULTIES = void 0;
...
function classifyQuery(query, sessionFacultyContext) {
    const q = query.toLowerCase().trim();
    // ── Follow-up detection ──
    const is_follow_up = FOLLOW_UP_PATTERNS.some(p => p.test(q));
    // ── Topic tags detection ──
    const topic_tags = [];
    for (const [tag, aliases] of Object.entries(exports.TOPIC_ALIASES)) {
    ...
    for (const [fid, keywords] of Object.entries(exports.FACULTIES)) {
    ...
    if (hasTopics)
        query_classification_confidence += 0.25;
    if (hasKeywords)
        query_classification_confidence += 0.15;
    if (faculty_detection_confidence >= 0.85)
        query_classification_confidence += 0.10;
    ...
    const needs_program_clarification = faculty_id === 'wnozk' &&
