...
const AnswerSchema = z.object({
  ...
  final_answer_confidence: z.number().min(0).max(1),
  ...
  response_type: z.enum(['answer', 'fallback', 'clarification', 'refusal']),
...
export async function generateAnswer(
  ...
  const context = chunks.map((c, i) =>
    `[Źródło ${i + 1}] ${c.title ?? ''} (${c.source_url})\n${c.text}`
  ).join('\n\n---\n\n');
  ...
  try {
    const response = await openai.chat.completions.create({
      model: MODEL,
      response_format: { type: 'json_schema', json_schema: JSON_SCHEMA as Parameters<typeof openai.chat.completions.create>[0]['response_format'] extends { json_schema: infer S } ? S : never },
      messages: [
    ...
    if (!raw) return buildFallback('Brak odpowiedzi od modelu.');
...
function buildFallback(reason: string): StructuredAnswer {
  return {
